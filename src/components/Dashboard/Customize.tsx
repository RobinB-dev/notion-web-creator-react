...
export const CustomizeMain = () => {
  ...
  const url = `${process.env.REACT_APP_BASE_UR_HEROKU}/notion_data`;
  ...
  useEffect(() => {
    ...
    const handleDataChange = () => {
      ...
      if (JSON.stringify(notionData) === "{}") {
        ...
        JSON.stringify(notionData) === JSON.stringify(data[0]) ||
  ...
  const { theme: pageTheme } = useCustomStyle("page", "a0c1294e-page");
  ...
  return (
    <div className={`${styles.notionPage} ${styles[pageTheme]}`}>
      ...
      {isStored &&
        _notionData.map((block: NotionBlock, index: number) =>
          CreateBlock(block, { index, activeAnim })
        )}
    </div>
  );
};
...
export const CustomizeToolBar = () => {
  const dataCtx = useContext(DataContext);
  let BlockType: string | undefined = testObj(
    menuToolBar,
    testObj(dataCtx.activeBlock, "obj")
  );
  // const BlockObj = testObj(dataCtx.activeBlock, "obj")

  BlockType === undefined && (BlockType = "general");

  useEffect(() => {}, [dataCtx.activeBlock]);
  ...
  return (
    <>
      ...
      <p>
        You can edit the looks of your page here ! <br></br>Click on a category
        to open it, change the values and see how your page will look in real
        time !
      </p>
      <p>{BlockType}</p>
